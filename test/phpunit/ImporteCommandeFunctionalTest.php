...
/**
 * \file    test/functional/ImporteCommandeFunctionalTest.php
...
/**
 * Class ImporteCommandeFunctionalTest
 *
 ...
 *
 * @package Testimportecommande
 */
class ImporteCommandeFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
...
//        )
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modImporteCommande')->click();
		} else {
			// Disable the module
			$this->byHref('modImporteCommande')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/importecommande/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('importecommande/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/importecommande/admin/about.php');
		$this->authenticate();
		return $this->assertContains('importecommande/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('importecommandewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modImporteCommande_ImporteCommandeTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modImporteCommande_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
